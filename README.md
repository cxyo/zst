...
图表生成采用链式配置模式。首先创建Line对象并设置初始化参数，包括图表主题、尺寸和页面标题。然后依次添加X轴数据、Y轴数据系列，每添加一个系列都配置相应的样式选项。最后调用set_global_opts方法设置全局配置，包括标题、提示框、图例、坐标轴、数据缩放控件和工具箱等。这种分步配置的方式使得代码逻辑清晰，便于理解和维护。
...
安装完成后，用户可以通过以下方式运行程序。在命令行中进入项目目录，执行python app.py命令即可启动程序。程序启动后会依次执行数据获取、数据处理和图表生成三个阶段，每个阶段都会在控制台输出相应的日志信息。正常情况下，程序会在当前目录下生成一个名为index.html的文件，该文件包含了完整的交互式图表。
...
项目目录结构简洁明了，包含以下文件和文件夹。app.py是主程序文件，包含了所有业务逻辑代码。old_app.py是原始版本的程序文件，保留了历史版本供参考对比。requirements.txt记录了项目依赖的第三方库及其版本要求。index.html是程序运行时生成的图表输出文件，每次运行都会重新生成。
...
本项目的面向对象设计使得扩展开发变得简单直接。如果需要添加新的数据源，可以创建继承自FundDataFetcher的新类，实现相同的接口方法。如果需要添加新的图表类型，可以参考FundChartGenerator类的设计，创建新的图表生成器类。如果需要添加定时自动运行功能，可以使用Windows任务计划程序或Linux cron定时任务来定期执行程序脚本。如果需要实现Web服务化部署，可以将图表生成逻辑封装为API接口，使用Flask或Django框架提供Web服务。
...
当前版本为2.0，相比1.0版本进行了多项重要改进。代码结构从面向过程重构为面向对象，提高了代码的可维护性和可扩展性。添加了日志系统，替代原有的print语句输出，便于问题排查和程序监控。修复了回调函数生成的bug，确保API请求参数的一致性。完善了类型注解，提高了代码的可读性和IDE支持。添加了完整的文档注释，增强了代码的自解释性。
